...
func TestCommanderMixedInboundUserManager(t *testing.T) {
	cmdPort := tcp.PickPort()
	mixedPort := tcp.PickPort()
	...
	serverConfig := &core.Config{
		...
		Inbound: []*core.InboundHandlerConfig{
			{
				ReceiverSettings: serial.ToTypedMessage(&proxyman.ReceiverConfig{
					PortList: &net.PortList{Range: []*net.PortRange{net.SinglePortRange(mixedPort)}},
					Listen:   net.NewIPOrDomain(net.LocalHostIP),
		...
		Operation: serial.ToTypedMessage(&command.AddUserOperation{
			User: &protocol.User{
				...
				Account: serial.ToTypedMessage(&socks.Account{
					Username: "mixed-user@example.com",
					Password: "passw0rd",
				}),
				BandwidthBps: 1234,
				ConnLimit:    5,
	...
	common.Must(err)
	if resp == nil {
		t.Fatal("nil add response")
	}

	target := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		_, _ = io.WriteString(w, "mixed-http-ok")
	}))
	defer target.Close()
	proxyURL := &url.URL{
		Scheme: "http",
		Host:   fmt.Sprintf("127.0.0.1:%d", mixedPort),
		User:   url.UserPassword("mixed-user@example.com", "passw0rd"),
	}
	client := &http.Client{
	...
	httpResp, err := client.Get(target.URL)
	common.Must(err)
	body, err := io.ReadAll(httpResp.Body)
	common.Must(err)
	common.Must(httpResp.Body.Close())
	if string(body) != "mixed-http-ok" {
		t.Fatalf("mixed HTTP response = %q", body)
	}

	users, err := hsClient.GetInboundUsers(context.Background(), &command.GetInboundUserRequest{Tag: "mixed"})
	common.Must(err)
	if len(users.GetUsers()) != 1 {
		t.Fatalf("expected one mixed user, got %d", len(users.GetUsers()))
	}
	got := users.GetUsers()[0]
	if got.GetEmail() != "mixed-user@example.com" || got.GetBandwidthBps() != 1234 || got.GetConnLimit() != 5 {
		t.Fatalf("mixed user mismatch: %+v", got)
	}

	count, err := hsClient.GetInboundUsersCount(context.Background(), &command.GetInboundUserRequest{Tag: "mixed"})
	common.Must(err)
	if count.GetCount() != 1 {
		t.Fatalf("expected one mixed user count, got %d", count.GetCount())
	}

	resp, err = hsClient.AlterInbound(context.Background(), &command.AlterInboundRequest{
		Tag:       "mixed",
		Operation: serial.ToTypedMessage(&command.RemoveUserOperation{Email: "mixed-user@example.com"}),
	})
	common.Must(err)
	if resp == nil {
		t.Fatal("nil remove response")
	}
	count, err = hsClient.GetInboundUsersCount(context.Background(), &command.GetInboundUserRequest{Tag: "mixed"})
	common.Must(err)
	if count.GetCount() != 0 {
		t.Fatalf("expected empty mixed user count after remove, got %d", count.GetCount())
	}
}
