...
// Drain closes listeners without closing the proxy/mux used by established
// sessions. The process supervisor owns the final Close after its drain TTL.
func (h *AlwaysOnInboundHandler) Drain() error {
	var errs []error
	for _, worker := range h.workers {
		errs = append(errs, worker.Drain())
	}
	return errors.Combine(errs...)
}

func (h *AlwaysOnInboundHandler) Resume() error {
	for _, worker := range h.workers {
		if err := worker.Start(); err != nil {
			return err
		}
