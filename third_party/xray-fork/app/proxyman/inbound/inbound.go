...
// DrainHandler stops accepting new sessions while retaining the handler in
// memory so established sessions can finish before the supervisor exits xray.
func (m *Manager) DrainHandler(ctx context.Context, tag string) error {
	if tag == "" {
		return common.ErrNoClue
	}
	m.access.Lock()
	...
	drainer, ok := handler.(interface{ Drain() error })
	if !ok {
...
func (m *Manager) ResumeHandler(ctx context.Context, tag string) error {
	if tag == "" {
		return common.ErrNoClue
	}
	m.access.Lock()
	...
	resumer, ok := handler.(interface{ Resume() error })
	if !ok {
		return errors.New("handler does not support listener resume: ", tag)
	}
