...
import (
	...
	"github.com/xtls/xray-core/app/accesslog"
...
// AccessLogConfig 是 ipipx 魔改：访问聚合器 app（进程内 hook 喂入 + PullAccess 拉取）。
// 配置块仅作开关：出现即实例化聚合器、注册 dispatcher hook、注入 command puller（仿 stats:{}）。
type AccessLogConfig struct {
	ChanSize         int32 `json:"chanSize"`
	MaxSitesPerEmail int32 `json:"maxSitesPerEmail"`
}

// Build implements Buildable.
func (c *AccessLogConfig) Build() (*accesslog.Config, error) {
	return &accesslog.Config{
		ChanSize:         c.ChanSize,
		MaxSitesPerEmail: c.MaxSitesPerEmail,
	}, nil
}
...
func (c *Config) Build() (*core.Config, error) {
	...
	if c.AccessLog != nil {
		accessLogConf, err := c.AccessLog.Build()
		if err != nil {
			return nil, errors.New("failed to build accesslog configuration").Base(err)
		}
		config.App = append(config.App, serial.ToTypedMessage(accessLogConf))
	}
