...
func (h *Handler) Process(ctx context.Context, link *transport.Link, dialer internet.Dialer) error {
	...
	defer conn.Close()
	// ipipx 旁路：回填真正拨通的对端 IP，供访问日志补齐 destIP（客户端送域名时这是唯一可信目标 IP）。
	if ra := net.DestinationFromAddr(conn.RemoteAddr()).Address; ra != nil && ra.Family().IsIP() {
		ob.DialedRemoteAddr = ra
	}
