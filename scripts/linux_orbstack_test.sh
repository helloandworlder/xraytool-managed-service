...
docker run --rm -v "${ROOT_DIR}:/work/xraytool" -w /work/xraytool golang:1.24 bash -lc "export PATH=/usr/local/go/bin:\$PATH GOTOOLCHAIN=auto && go test ./... && go build -buildvcs=false ./..."
